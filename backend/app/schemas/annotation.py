...
class ScoreConfigBase(BaseModel):
    ...
    name: str = Field(..., min_length=1, max_length=100, description="Name of the score config")
    description: Optional[str] = Field(None, description="Description of what this score measures")
    data_type: str = Field(
        default="numerical",
    ...
    min_value: Optional[float] = Field(0.0, description="Minimum value for numerical type")
    max_value: Optional[float] = Field(1.0, description="Maximum value for numerical type")
    binary_labels: Optional[BinaryLabels] = Field(None, description="Labels for binary type")
    annotation_level: str = Field(
        default="both",
        description="What can be annotated: 'trace', 'span', or 'both'"
    )
...
class AnnotationBase(BaseModel):
    ...
    string_value: Optional[str] = Field(None, description="String representation (for categorical)")
    comment: Optional[str] = Field(None, description="Optional comment")


class AnnotationCreate(AnnotationBase):
    """Schema for creating a new annotation."""
    source: str = Field(default="direct", description="Source: 'queue', 'direct', or 'api'")
    queue_item_id: Optional[str] = Field(None, description="Queue item ID if from queue")

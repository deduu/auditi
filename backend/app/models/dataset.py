...
class Dataset(Base):
    ...
    )  # manual, annotation_queue, import
    ...
    expected_output_schema = Column(
        JSON, nullable=True
    )  # JSON Schema for expected_output validation
...
class DatasetItem(Base):
    ...
    expected_output = Column(
        JSON, nullable=True
    )  # Expected/ideal output (ground truth)
    meta = Column(
        JSON, nullable=True
    )  # Additional item metadata (named 'meta' to avoid SQLAlchemy reserved name)
    ...
    # Version tracking
    version_added = Column(
        Integer, nullable=False, default=1
    )  # Dataset version when item was added
